...
class FusionGame {
  ...
  draw() {
    ...
    // Next preview (mobile, 100x100 canvas)
    const previewShapes = this.getShapes();
    if (this.nextCtx && this.nextShape < previewShapes.length) {
      this.nextCtx.clearRect(0, 0, 100, 100);
      // FD-002: bigger max radius now that the canvas is 100x100.
      const ns = previewShapes[this.nextShape];
      const scale = Math.min(1.0, 42 / Math.max(1, ns.radius));
      drawShape(this.nextCtx, 50, 50, this.nextShape, scale, previewShapes);
    }
    // Next preview (desktop, 140x140 canvas)
    if (this.nextCtxDesk && this.nextShape < previewShapes.length) {
      this.nextCtxDesk.clearRect(0, 0, 140, 140);
      const ns = previewShapes[this.nextShape];
      const scale = Math.min(1.4, 60 / Math.max(1, ns.radius));
      drawShape(this.nextCtxDesk, 70, 70, this.nextShape, scale, previewShapes);
    }
  }
